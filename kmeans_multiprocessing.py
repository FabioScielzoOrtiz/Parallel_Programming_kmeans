...
# Elbow computation (for 1.000.000 rows and k=2,...,12) (5.42 mins | 342 segs (new PC))
...
if __name__ == "__main__":
    ...
    pool = Pool(processes=n_processes)  # Adjust the number of processes as needed
    k_values = range(2, 13)
    results = pool.map(compute_elbow, k_values)
    pool.close()
    pool.join()
    WCSS_Elbow_dict = dict(results)
    end_time = time.time()
...
#######################################################################################
    ...
    scale_centroids_df.columns = computers_1000000.columns
